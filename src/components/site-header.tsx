...
export function SiteHeader() {
  ...
  return (
    <header className="border-line-dark bg-indigo-deep text-invert relative z-50 border-b">
      <TopUtilityBar />
      <Container className="flex h-16 items-center justify-between">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          ...
          {restNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-invert/80 hover:text-invert text-sm transition-colors"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-4 md:flex">
          ...
          <Link
            ...
            className="border-clay-soft/70 text-invert hover:border-clay hover:bg-clay border px-4 py-2 text-sm font-medium transition-colors"
          >
            Discuss a project
          </Link>
        </div>
